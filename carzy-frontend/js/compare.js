...
function loadCarsForSelection() {
    ...
    selectionGrid.innerHTML = '<div class="spinner"></div>';
    ...
    fetch('http://localhost:5000/api/cars')
        ...
        .catch(error => {
            ...
            selectionGrid.innerHTML = '<p>Failed to load car data. Please try again later.</p>';
        });
}
...
function performComparison() {
    ...
    .then(cars => {
        cars.forEach((car, index) => {
            ...
            document.getElementById(`car${index+1}-fuel`).textContent = car.fuel_type;
            ...
            document.getElementById(`car${index+1}-body`).textContent = car.body_type;
            document.getElementById(`car${index+1}-safety`).textContent = car.safety_rating || 'N/A';
            document.getElementById(`car${index+1}-boot`).textContent = car.boot_space || 'N/A';
            document.getElementById(`car${index+1}-ground`).textContent = car.ground_clearance || 'N/A';
            
            // Update features
            document.getElementById(`car${index+1}-ac`).textContent = car.features?.ac ? 'Yes' : 'No';
            document.getElementById(`car${index+1}-power-steering`).textContent = car.features?.power_steering ? 'Yes' : 'No';
            document.getElementById(`car${index+1}-power-windows`).textContent = car.features?.power_windows ? 'Yes' : 'No';
            ...
            document.getElementById(`car${index+1}-parking`).textContent = car.features?.parking_sensors ? 'Yes' : 'No';
    ...
    .catch(error => {
        ...
        compareResults.innerHTML = '<p>Failed to compare cars. Please try again later.</p>';
    });
}
